...
/**
 * ProfileDetailPage — view full details of another user's profile.
 * Shows: photos, name, age, city, education, profession, religion, hobbies.
 * Actions: Like ❤️ and Send Interest 💌.
 *
 * Phase 1 — no contact info reveal (phone/email hidden).
 */
const ProfileDetailPage = () => {
  const { profileId } = useParams();    // get profileId from URL: /profiles/:profileId
  const navigate = useNavigate();

  const [profile, setProfile] = useState(null);
  const [loading, setLoading] = useState(true);
  const [selectedPhoto, setSelectedPhoto] = useState(null); // currently displayed large photo
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        setProfile(data);
        // Set primary photo as the selected large photo by default
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      // Show backend error message (e.g. "already liked", "daily limit reached")
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="bg-white dark:bg-card-dark rounded-2xl shadow-sm overflow-hidden">
          ...
          {/* ── Thumbnail photo gallery (click to enlarge) ── */}
...
/** Small helper: label + value row for profile details */
